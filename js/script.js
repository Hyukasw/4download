...
// Show video info result card
function showResult(title, duration, size, quality, thumbHtml, previewHtml, platform, vid) {
  const result = document.getElementById('result');

  document.getElementById('resultTitle').textContent = title || 'Video';
  ...
  const qEl = document.getElementById('resultQuality');
  if (quality) { qEl.textContent = '\u2022 ' + quality; qEl.style.display = ''; }
  ...
  document.getElementById('resultPreview').innerHTML = previewHtml || '<span class="result-placeholder">Preview unavailable</span>';

  result.classList.add('show');
  setTimeout(() => result.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 300);
}
...
async function handleDownload() {
  ...
  document.getElementById('resultSize').style.display = 'none';
  ...
  // YouTube: fetch metadata via /api/info
  if (platform === 'youtube') {
    const vid = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/)?.[1];
    if (!vid) return;

    const thumbHtml = '<img src="/api/thumb?v=' + vid + '" style="width:100%;height:100%;object-fit:cover" alt="" onerror="this.outerHTML=\'<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:26px>\uD83C\uDFAC</div>\'">';
    const previewHtml = '<div style="width:100%;height:100%;position:relative"><iframe src="/api/embed?v=' + vid + '" allow="autoplay; encrypted-media" allowfullscreen style="width:100%;height:100%;border:none;"></iframe></div>';
    ...
    try {
      ...
      const label = data.maxQuality
        ? 'Up to ' + data.maxQuality + (data.best?.sizeFormatted ? ' \u2022 ' + data.best.sizeFormatted : '')
        : (data.best?.quality ? data.best.quality : null);

      showResult(
        data.title || 'YouTube Video',
        data.durationFormatted || '\u2014',
        data.best?.sizeFormatted || null,
        label,
        thumbHtml,
        previewHtml
      );
    } catch (e) {
      // Fallback: show basic info without size/duration
      showResult('YouTube Video', null, null, null, thumbHtml, previewHtml);
    }
    return;
  }

  // Non-YouTube: try to fetch info, show preview
  const icons = { tiktok: '\uD83C\uDFB5', instagram: '\uD83D\uDCF8', facebook: '\uD83D\uDCD8', twitter: '\uD835\uDD4E', snapchat: '\uD83D\uDC7B', vimeo: '\uD83C\uDFA5', dailymotion: '\uD83D\uDCFA', direct: '\uD83D\uDCF9' };
  const icon = icons[platform] || '\uD83C\uDFAC';
  const labels = { tiktok: 'TikTok', instagram: 'Instagram', facebook: 'Facebook', twitter: 'Twitter / X', snapchat: 'Snapchat', vimeo: 'Vimeo', dailymotion: 'Dailymotion', direct: 'Direct Video' };
  const label = labels[platform] || 'Video';

  // Try to fetch info (non-YouTube)
  ...
  const previewHtml = '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:16px;padding:24px;text-align:center;background:radial-gradient(ellipse at center, rgba(99,102,241,.05), transparent)">'
    ...
    + '<span style="padding:4px 12px;border-radius:100px;background:var(--grad-dim);background:linear-gradient(135deg, rgba(99,102,241,.15), rgba(139,92,246,.15));color:var(--g1);font-size:11px;font-weight:600">HD</span>'
    + '<span style="padding:4px 12px;border-radius:100px;background:var(--grad-dim);background:linear-gradient(135deg, rgba(99,102,241,.15), rgba(139,92,246,.15));color:var(--g2);font-size:11px;font-weight:600">Full HD</span>'
    + '<span style="padding:4px 12px;border-radius:100px;background:var(--grad-dim);background:linear-gradient(135deg, rgba(99,102,241,.15), rgba(139,92,246,.15));color:var(--g3);font-size:11px;font-weight:600">4K</span>'
    + '</div></div>';

  showResult(title, null, null, null,
