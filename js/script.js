...
let lastUrl = '', lastPlatform = '', lastThumbUrl = '';
...
document.addEventListener('click', e => { if (!e.target.closest('.lang-menu')) document.getElementById('langDropdown').classList.remove('open'); });
...
let vidIdx = 0;
function cycleBg() { vidIdx = (vidIdx + 1) % sources.length; bgVid.src = sources[vidIdx]; bgVid.play().catch(() => {}); }
bgVid.src = sources[0]; bgVid.play().catch(() => {});
...
function getUserInfo(url, platform) {
  if (platform === 'tiktok') { const m = url.match(/@([a-zA-Z0-9_.]+)/); return m ? m[1] : null; }
  if (platform === 'instagram') { const m = url.match(/(?:instagram\.com\/)([a-zA-Z0-9_.]+)/); return m && !m[1].startsWith('reel') && !m[1].startsWith('p') ? m[1] : null; }
  return null;
}
...
const PLATFORM_META = {
  ...
  direct: { icon: '\uD83D\uDCF9', label: 'Direct Video' },
};
...
function handleDownload() {
  ...
  const pm = PLATFORM_META[platform] || { icon: '\uD83C\uDFAC', label: 'Video' };
  ...
  document.getElementById('resultTitle').textContent = pm.label + ' Video' + (username ? ' \u2014 @' + username : ' \u2014 ' + shortUrl);
  document.getElementById('resultThumb').innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:28px">' + pm.icon + '</div>';
  document.getElementById('resultPreview').innerHTML = '<div class="spinner-wrap"><div class="spinner"></div></div>';
  document.getElementById('result').classList.add('show');
  lastThumbUrl = '';

  // For YouTube, show iframe immediately
  if (platform === 'youtube') {
    const vid = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/)?.[1];
    if (vid) {
      document.getElementById('resultThumb').innerHTML = '<img src="https://img.youtube.com/vi/' + vid + '/maxresdefault.jpg" style="width:100%;height:100%;object-fit:cover" alt="">';
      document.getElementById('resultPreview').innerHTML = '<iframe src="https://www.youtube.com/embed/' + vid + '?autoplay=1&mute=1" allow="autoplay; encrypted-media" allowfullscreen style="width:100%;height:100%;border:none;"></iframe>';
      document.getElementById('resultDuration').textContent = '\u2026';
      document.getElementById('resultQuality').textContent = '';
      // Fetch YouTube metadata
      fetch(SERVER + '/yt?v=' + vid, { method: 'HEAD' }).catch(() => {});
    }
  }
...
async function doDownload(btn, restoreText) {
  ...
  if (plat === 'youtube') {
    const vid = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/)?.[1];
    if (vid) {
      btn.textContent = '\u23F3 Downloading YouTube...';
      triggerDownload('/yt?v=' + vid);
      setTimeout(() => { btn.textContent = restoreText; btn.disabled = false; }, 2000);
      return;
    }
  }

  btn.textContent = '\u23F3 Fetching video...';
  triggerDownload('/dl?url=' + encodeURIComponent(url));
  setTimeout(() => { btn.textContent = restoreText; btn.disabled = false; }, 2000);
}
