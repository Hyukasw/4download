...
// ─── API calls (relative, same-origin, no CORS) ───
...
function handleDownload() {
  ...
  if (platform === 'youtube') {
    ...
    if (vid) {
      document.getElementById('resultThumb').innerHTML = '<img src="/api/thumb?v=' + vid + '" style="width:100%;height:100%;object-fit:cover" alt="" onerror="this.outerHTML=\'<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:28px>' + pm.icon + '</div>\'">';
...
async function doDownload(btn, restoreText) {
  ...
  apiDownload('/api/dl?url=' + encodeURIComponent(url));
  setTimeout(() => { btn.textContent = restoreText; btn.disabled = false; }, 2000);
}
